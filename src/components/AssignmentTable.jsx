...
import { formatDate, formatCurrency, calcProfit, isOverdue, isDueToday, daysUntilDue } from '../utils/helpers.js'
  ...
  return (
    <div>
      ...
      {status !== 'Completed' && days !== null && (
        <p className={`text-xs mt-0.5 ${overdue ? 'text-rose-500' : today ? 'text-amber-500' : 'text-slate-400'}`}>
          {overdue
      ...
      <td colSpan={99} className="px-6 pb-4 pt-0">
        <div className="ml-0 p-4 rounded-xl bg-slate-50 dark:bg-slate-700/30 border border-slate-200 dark:border-slate-600 text-sm grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-3">
          ...
          <div>
            ...
            <p className="text-slate-700 dark:text-slate-200">{a.subcontractorName || '—'}</p>
          </div>
          <div>
    ...
    <>
      ...
      >
        ...
        <td className="px-4 py-3 min-w-[120px]">
          <DueDateCell dueDate={a.dueDate} status={a.assignmentStatus} />
        </td>
        ...
        <td className="px-4 py-3 min-w-[130px]">
          <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">
            {formatCurrency(a.subcontractorPrice)}
          </p>
